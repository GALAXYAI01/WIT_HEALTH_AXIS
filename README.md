...
| `WIT_ADMIN_EMAIL` / `WIT_ADMIN_PASSWORD` | Recovery administrator credentials. Set both in Render; the account is created or restored as `admin` on startup. |
...
.venv\Scripts\python.exe auth_persistence_test.py
...
- `WIT_ADMIN_EMAIL` and `WIT_ADMIN_PASSWORD`: the recovery administrator credentials. Keep these in Render Environment, never in Git.
...
The same database stores authentication accounts. On the free demo service, a cold restart can therefore remove accounts created only through the browser. Configure `WIT_ADMIN_EMAIL` and `WIT_ADMIN_PASSWORD` in Render so the named recovery administrator is recreated or promoted to `admin` on every startup. For every user-created account and scan record to survive restarts, use the persistent disk or managed Postgres option above.
