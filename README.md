...
The three model weights are versioned in `model_weights/` because the inference service loads them at startup. Render's free filesystem is ephemeral, so scan history in the Blueprint's `/tmp` database is suitable for a demo only. Use a paid persistent disk mounted at `/var/data` with `WIT_HISTORY_DB_PATH=/var/data/wit-scan-history.db`, or migrate the history layer to managed Postgres, before relying on history as an audit record. The Torch models also need a service plan with enough memory to load all three models.
